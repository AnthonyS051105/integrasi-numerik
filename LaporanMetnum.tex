...
\begin{center}
    ...
    \vspace{0.1cm}
    \textbf{1. Nathanael Satya Saputra (24/534424/TK/59236)} \\
...
Integrasi analitik adalah metode perhitungan integral menggunakan rumus-rumus kalkulus secara langsung. Metode ini memberikan nilai eksak yang tepat dari suatu integral jika fungsi yang diintegralkan memiliki antiturunan yang dapat ditentukan.
...
Metode \textit{Richardson Extrapolation} merupakan metode numerik yang digunakan untuk meningkatkan akurasi hasil pendekatan integral dengan cara mengkombinasikan dua hasil aproksimasi berbeda. Ide utama dari metode ini adalah memanfaatkan hubungan antara galat (\textit{error}) dan ukuran langkah $h$.

Jika diketahui dua pendekatan integral $I_h$ dan $I_{h/2}$ yang diperoleh dari \textit{Trapezoidal Rule} dengan \textit{step size} $h$ dan $h/2$, maka nilai integral sebenarnya $I$ dapat diestimasi dengan formula:

\[
I \approx \frac{4I_{h/2} - I_h}{3}
\]

Dengan menggunakan metode ini, akurasi perhitungan dapat meningkat tanpa harus meningkatkan jumlah pembagian secara drastis, sehingga efisien baik dalam waktu komputasi maupun sumber daya.

\textbf{Algoritma:}

\begin{verbatim}
Input: fungsi f(x), batas a dan b, jumlah segmen awal n

1. Hitung I_h = trapezoidal_rule(f, a, b, n)
2. Hitung I_h2 = trapezoidal_rule(f, a, b, 2 * n)
3. Lakukan extrapolasi Richardson tingkat pertama:
I = (4 * I_h2 - I_h) / 3
4. Kembalikan hasil I sebagai aproksimasi 
   integral dengan akurasi lebih tinggi
Output: nilai I hasil extrapolasi
\end{verbatim}
...
# Contoh penggunaan
richardson = richardson_extrapolation(f, 0, np.pi/2, n=4)
\end{lstlisting}

\subsection*{1.4. Metode Romberg Integration}

\textit{Romberg integration} adalah metode integrasi numerik yang menggabungkan Trapezoidal Rule dengan Richardson Extrapolation. Perbedaan utama dengan Richardson standar adalah cara menghitung kolom pertama menggunakan teknik rekursif yang lebih efisien.

Adapun rumus dari metode romberg didefinisikan sebagai berikut.
...
\textit{Adaptive integration} adalah metode integrasi numerik yang secara adaptif menyesuaikan ukuran interval berdasarkan fungsi. Metode ini menggunakan Simpson's Rule sebagai basis dan membagi interval menjadi subinterval yang lebih kecil sampai mencapai toleransi error yang ditentukan.

Rumus dari simpson's rule pada adaptive integration didefinisikan sebagai berikut.
...
Adapun kriteria adaptif sendiri yakni sebagai berikut.
...
\textit{Gaussian quadrature} adalah metode integrasi numerik yang menggunakan titik-titik sampling optimal \textit{nodes} dan bobot \textit{weights} yang telah ditentukan secara matematis untuk memberikan akurasi maksimal dengan jumlah evaluasi fungsi minimal.

Rumus dari metode \textit{Gaussian quadrature} yakni sebagai berikut.
Untuk interval $[-1, 1]$:
\[
...
Transformasi ke Interval $[a, b]$ untuk interval $[a, b]$, menggunakan transformasi:
\[
...
S = \frac{h}{6}[f(a) + 4f(c) + f(b)]
\]
di mana $h = b - a$ adalah lebar interval. Rumus ini memberikan hasil eksak untuk polinomial berderajat $\leq 3$, sehingga sangat akurat untuk fungsi yang smooth.
...
Pertama, impor library numpy untuk perhitungan numerik:

\begin{lstlisting}
import numpy as np
\end{lstlisting}

\textbf{Implementasi Python untuk Fungsi 1: } $f(x) = \cos(x)$ pada $[0, \frac{\pi}{2}]$
...
Inisialisasi parameter integral untuk fungsi 1. Pada kasus ini $a = 0$ adalah batas bawah, $b = \frac{\pi}{2} \approx 1.5708$ adalah batas atas, $\text{tol} = 10^{-10}$ adalah toleransi error yang sangat kecil, dan $\text{max\_depth} = 50$ adalah kedalaman rekursi maksimal.
...
Hitung titik tengah dan nilai fungsi pada tiga titik penting. Untuk fungsi $\cos(x)$: $c = \frac{\pi}{4} \approx 0.7854$, dengan $f(0) = 1.0$, $f(\pi/2) = 0.0$, dan $f(\pi/4) \approx 0.7071$.
...
Fungsi Simpson's Rule untuk interval $[a, b]$. Untuk $\cos(x)$: $h = \frac{\pi}{2} \approx 1.5708$, sehingga $S \approx 1.0023$ (aproksimasi awal dengan error sekitar 0.0023 dari nilai eksak 1.0).
...
Fungsi rekursif adaptive menghitung 2 titik tambahan untuk membagi interval. Untuk $\cos(x)$ pada iterasi pertama: $d = \frac{\pi}{8} \approx 0.3927$ dan $e = \frac{3\pi}{8} \approx 1.1781$, dengan $f(d) \approx 0.9239$ dan $f(e) \approx 0.3827$.
...
Hitung Simpson's Rule untuk kedua subinterval. Untuk $\cos(x)$: $S_{\text{left}} \approx 0.5236$, $S_{\text{right}} \approx 0.4767$, total $S_2 \approx 1.0003$.
...
Cek kriteria konvergensi. Untuk $\cos(x)$: $|S_2 - S| = 0.0020$ dan $15 \times \text{tol} = 1.5 \times 10^{-9}$. Karena $0.0020 > 1.5 \times 10^{-9}$, belum konvergen. Jika sudah konvergen, kembalikan hasil dengan koreksi Richardson.
...
Jika belum konvergen, lakukan rekursi pada kedua subinterval. Proses ini terus berlanjut dengan toleransi dibagi 2 dan kedalaman berkurang 1 sampai konvergensi tercapai.

\vspace{0.3cm}

\textbf{Implementasi Python untuk Fungsi 2: } $f(x) = x^2$ pada $[0, 1]$
...
Inisialisasi parameter integral untuk fungsi 2. Pada kasus ini $a = 0$ adalah batas bawah, $b = 1$ adalah batas atas, $\text{tol} = 10^{-10}$ adalah toleransi error, dan $\text{max\_depth} = 50$ adalah kedalaman rekursi maksimal.
...
Hitung titik tengah dan nilai fungsi pada tiga titik penting. Untuk fungsi $x^2$: $c = 0.5$, dengan $f(0) = 0.0$, $f(1) = 1.0$, dan $f(0.5) = 0.25$.

\begin{lstlisting}[basicstyle=\ttfamily\footnotesize]
S = simpson_rule(f, a, b)
\end{lstlisting}

Fungsi Simpson's Rule untuk interval $[0, 1]$. Untuk $x^2$: $h = 1$, sehingga $S = \frac{1}{6}[0.0 + 4(0.25) + 1.0] = 0.333333...$ (sudah sangat akurat karena Simpson's Rule eksak untuk polinomial kuadrat).
...
Fungsi rekursif adaptive menghitung 2 titik tambahan. Untuk $x^2$ pada iterasi pertama: $d = 0.25$ dan $e = 0.75$, dengan $f(d) = 0.0625$ dan $f(e) = 0.5625$.
...
Hitung Simpson's Rule untuk kedua subinterval. Untuk $x^2$: $S_{\text{left}} = 0.041666...$, $S_{\text{right}} = 0.291666...$, total $S_2 = 0.333333...$.
...
Cek kriteria konvergensi. Untuk $x^2$: $|S_2 - S| \approx 0$ (sangat kecil) dan $15 \times \text{tol} = 1.5 \times 10^{-9}$. Karena selisihnya sudah sangat kecil, kondisi terpenuhi dan algoritma langsung konvergen pada iterasi pertama.

\vspace{0.3cm}

\textbf{Hasil Akhir:}

\subsection*{Fungsi 1: $\int_{0}^{\frac{\pi}{2}} \cos(x)  dx$}

Integral fungsi $\cos(x)$ pada interval $[0, \frac{\pi}{2}]$ memiliki nilai eksak $1.000000000000000$. Hasil aproksimasi dengan metode Adaptive Simpson:

\begin{itemize}
    \item \textbf{Adaptive Simpson}: Nilai $1.000000000000000$ dengan error $0.000000 \times 10^{0}$
\end{itemize}

\subsection*{Fungsi 2: $\int_{0}^{1} x^2  dx$}

Integral fungsi $x^2$ pada interval $[0, 1]$ memiliki nilai eksak $0.333333333333333$. Hasil aproksimasi dengan metode Adaptive Simpson:

\begin{itemize}
    \item \textbf{Adaptive Simpson}: Nilai $0.333333333333333$ dengan error $0.000000 \times 10^{0}$
\end{itemize}
...
\textbf{F            ungsi 2:} $f(x) = x^2$ pada interval $[0, 1]$
...
\begin{enumerate}
    ...
    Berdasarkan hasil pada Tabel 1 dan Tabel 2, metode Adaptive Integration menunjukkan tingkat akurasi yang sangat tinggi, dengan error yang hampir tidak terlihat. Pada fungsi \( f(x) = \cos(x) \), error-nya hanya sekitar \(1.11\times10^{-15}\), sedangkan pada fungsi \( f(x) = x^2 \) errornya yakni nol.  Dari hasil yang diperoleh, dapat dikatakan bahwa metode Adaptive Integration mampu memberikan hasil aproksimasi yang sangat presisi.

    \item \textbf{Gaussian Quadrature} \\
    Jika dilihat dari hasil pada kedua tabel, metode Gaussian Quadrature juga memberikan hasil yang sangat akurat. Untuk fungsi \( f(x) = \cos(x) \), error-nya hanya sekitar \(3.96\times10^{-11}\), sedangkan untuk \( f(x) = x^2 \) nilainya tepat sama dengan hasil eksak. Meskipun error-nya sedikit lebih besar dibandingkn $\cos(x)$ metode Adaptive Integration, secara keseluruhan metode ini tetap dapat memberikan hasil yang sangat akurat.
...
\begin{enumerate}
    \item Besarnya error sangat bergantung pada orde, parameter, dan karakteristik metode yang digunakan. Metode dengan orde rendah seperti \textit{Trapezoidal Rule} memiliki nilai error yang lebih besar, namun akurasinya dapat ditingkatkan dengan menambah jumlah subinterval (\(n\)).
    ...
    \item Metode \textit{Richardson Extrapolation} dan \textit{Romberg Integration} mampu meningkatkan akurasi hasil integrasi dibandingkan metode Trapezoidal. Kombinasi hasil perhitungan sebelumnya untuk memperkirakan nilai dapat menghasilkan hasil yang lebih akurat terutama pada metode romberg yang melakukannya secara iterasi terus-menerus.
    
    \item Metode \textit{Adaptive Integration}
    Metode Adaptive Integration menunjukkan hasil paling akurat dengan error yang sangat kecil hingga mendekati nol di kedua kasus.

    \item Metode \textit{Gaussian Quadrature}
    Metode Gaussian Quadrature menunjukkan hasil yang juga sangat akurat, namun jika dibandingkan dengan Adaptive Integration, Gaussian Quadrature masih menghasilkan error yang lebih tinggi.
    
    
    \item \textit{Adaptive Integration} dan \textit{Gaussian Quadrature} merupakan metode yang paling efisien dan akurat, sementara metode Trapezoidal lebih cocok digunakan untuk perhitungan sederhana atau sebagai dasar pengembangan metode orde lebih tinggi.
\end{enumerate}
