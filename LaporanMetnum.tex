...
\begin{center}
    ...
    \textbf{3. Yohanes Anthony Saputra (24/536237/TK/59524)} \\
...
\textbf{Soal 1: } $f(x) = \cos(x)$ pada interval $[0, \frac{\pi}{2}]$
...
\textbf{Soal 2: } $f(x) = x^2$ pada interval $[0, 1]$
...
\begin{itemize}
    \item f: fungsi yang akan diintegralkan, yaitu soal 1: $f(x) = \cos(x)$ dan soal 2: $f(x) = x^2$
    \item a: batas bawah, yaitu $0$ untuk soal 1 dan soal 2
    \item b: batas atas, yaitu $\frac{\pi}{2}$ untuk soal 1 dan $1$ untuk soal 2
...
\includegraphics[width=16cm]{Trapezoidal.png}
...
\includegraphics[width=16cm]{TrapezoidalX2.png}
...
\textbf{Langkah perhitungan dengan program Python:}
...
Kemudian, mendeklarasikan fungsi untuk melakukan perhitungan metode \textit{Richardson Extrapolation}. Fungsi ini menerima input:
\begin{itemize}
    \item \textbf{f}: fungsi yang akan diintegralkan, yaitu soal 1: $f(x) = \cos(x)$ dan soal 2: $f(x) = x^2$
    \item \textbf{a}: batas bawah, yaitu 0 untuk soal 1 dan soal 2
    \item \textbf{b}: batas atas, yaitu $\frac{\pi}{2}$ untuk soal 1 dan 1 untuk soal 2
    \item \textbf{n}: jumlah subinterval awal, yaitu $n = 4$
\end{itemize}
...
Pada implementasi sederhana ini (berbeda dengan implementasi tabel penuh di bagian teori), fungsi \textit{Richardson Extrapolation} melakukan ekstrapolasi tingkat pertama dengan menggabungkan dua hasil \textit{trapezoidal rule} yang berbeda.

Langkah pertama adalah menghitung integral menggunakan \textit{Trapezoidal Rule} dengan $n$ subinterval:

\begin{lstlisting}
    I_h = trapezoidal_rule(f, a, b, n)
\end{lstlisting}

Nilai $I_h$ adalah aproksimasi integral dengan ukuran langkah $h = \frac{b-a}{n}$. Hasil ini memiliki error dalam bentuk:
$$I_h = I_{\text{eksak}} + c_1 h^2 + c_2 h^4 + \cdots$$

Langkah kedua adalah menghitung integral menggunakan \textit{Trapezoidal Rule} dengan $2n$ subinterval (ukuran langkah setengahnya):

\begin{lstlisting}
    I_h2 = trapezoidal_rule(f, a, b, 2 * n)
\end{lstlisting}

Nilai $I_{h/2}$ adalah aproksimasi integral dengan ukuran langkah $\frac{h}{2} = \frac{b-a}{2n}$. Hasil ini memiliki error:
$$I_{h/2} = I_{\text{eksak}} + c_1 \left(\frac{h}{2}\right)^2 + c_2 \left(\frac{h}{2}\right)^4 + \cdots$$
$$I_{h/2} = I_{\text{eksak}} + \frac{c_1 h^2}{4} + \frac{c_2 h^4}{16} + \cdots$$

Langkah ketiga adalah melakukan ekstrapolasi Richardson untuk mengeliminasi error orde $O(h^2)$:
...
Integral fungsi $\cos(x)$ pada interval $[0, \frac{\pi}{2}]$ memiliki nilai eksak $1.00000000000000$. Berikut hasil aproksimasi dengan metode \textit{Richardson Extrapolation} menggunakan $n=4$:

\includegraphics[width=16cm]{Richardson.png}

\subsection*{Soal 2: $\int_{0}^{1} x^2 dx$}

Integral fungsi $x^2$ pada interval $[0, 1]$ memiliki nilai eksak $0.333333333333333$. Berikut hasil aproksimasi dengan metode \textit{Richardson Extrapolation} menggunakan $n=4$:

\includegraphics[width=16cm]{RichardsonX2.png}
...
\begin{itemize}
    \item \textbf{f}: fungsi yang akan diintegralkan, soal 1: $f(x) = \cos(x)$ dan soal 2: $f(x) = x^2$
    \item \textbf{a}: batas bawah integral, yaitu 0 untuk soal 1 dan soal 2
    \item \textbf{b}: batas atas integral, yaitu $\frac{\pi}{2}$ untuk soal 1 dan $1$ untuk soal 2
...
\includegraphics[width=16cm]{RombergTabel.png}

\includegraphics[width=16cm]{Romberg.png}
...
\includegraphics[width=16cm]{RombergTabelX2.png}

\includegraphics[width=16cm]{RombergX2.png}
...
\includegraphics[width=16cm]{Adaptive.png}
...
\includegraphics[width=16cm]{AdaptiveX2.png}
...
\textbf{Soal 1:} $f(x) = \cos(x)$ untuk interval $[0, \frac{\pi}{2}$]
...
\includegraphics[width=16cm]{Gaussian.png}
\begin{align*}
...
\includegraphics[width=16cm]{GaussianX2.png}
\begin{align*}
...
\begin{enumerate}
    ...
    \item \textbf{Richardson Extrapolation} \\
       Jika diamati pada kedua tabel diatas, Richardson Extrapolation bisa untuk menurunkan error dengan cukup signifikan baik pada kasus $f(x) = \cos(x)$ ataupun $f(x) = x^2$. Ini menunjukkan bahwa metode Richardson dapat memberikan hasil aproksimasi yang lebih dekat dan error yang lebih kecil dibandingkan dengan \textit{Trapezoidal Rule} biasa. Untuk fungsi $\cos(x)$, error berkurang dari $\sim 10^{-2}$ (\textit{Trapezoidal} n=4) menjadi $\sim 10^{-6}$ (\textit{Richardson}). Untuk fungsi polinomial $x^2$, \textit{Richardson Extrapolation} menghasilkan nilai eksak karena dapat mengeliminasi seluruh error untuk fungsi dengan turunan ketiga nol.
